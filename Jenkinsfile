  ...
  stages {
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep build-chain | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            TAG=$feature_name DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
          fi
          images=`docker images | grep entropypool | grep build-chain | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep build-chain | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          if [ 0 -eq $rc -a x"$revlist" != x ]; then
            tag=`git describe --tags $revlist`
            set +e
            docker images | grep build-chain | grep $tag
            rc=$?
            set -e
            if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/build-chain:latest/build-chain:$feature_name/g" cmd/build-chain/k8s/02-build-chain.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/build-chain/k8s/02-build-chain.yaml
          sed -i "s#currency_proxy: \\\"\\\"#currency_proxy: \\\"$CURRENCY_REQUEST_PROXY\\\"#g" cmd/build-chain/k8s/00-configmap.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/build-chain/k8s/02-build-chain.yaml'
        sh 'sed -i "s#currency_proxy: \\\"\\\"#currency_proxy: \\\"$CURRENCY_REQUEST_PROXY\\\"#g" cmd/build-chain/k8s/00-configmap.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/build-chain:latest/build-chain:$tag/g" cmd/build-chain/k8s/02-build-chain.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/build-chain/k8s/02-build-chain.yaml
          sed -i "s#currency_proxy: \\\"\\\"#currency_proxy: \\\"$CURRENCY_REQUEST_PROXY\\\"#g" cmd/build-chain/k8s/00-configmap.yaml
